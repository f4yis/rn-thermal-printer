...
public class BLEPrinterAdapter implements PrinterAdapter{
    ...
    private final static byte[] SET_LINE_SPACE_24 = new byte[] { ESC_CHAR, 0x33, 0 };
    private final static byte[] SET_LINE_SPACE_32 = new byte[] { ESC_CHAR, 0x33, 0 };
    ...
    public static Bitmap resizeTheImageForPrinting(Bitmap image) {
        ...
        int height = image.getHeight();
        if (width > 575) {
            float decreaseSizeBy = (575.0f / width);
            return getBitmapResized(image, decreaseSizeBy);
            // if (width > height) {
            //     float decreaseSizeBy = (600.0f / width);
            //     return getBitmapResized(image, decreaseSizeBy);
            // } else {
            //     float decreaseSizeBy = (600.0f / height);
            //     return getBitmapResized(image, decreaseSizeBy);
            // }
        }
        return image;
